...
class Event(models.Model):
    ...
    ticket = models.BooleanField(default=False, help_text="Does this event require a ticket?")
    ticket_price = models.PositiveIntegerField(null=True, blank=True, help_text="Price in NPR (only if ticket is required)")
    
    def clean(self):
        """Custom validation to ensure ticket_price is only set when ticket is True"""
        from django.core.exceptions import ValidationError
        if self.ticket and not self.ticket_price:
            raise ValidationError({'ticket_price': 'Price is required when ticket is needed.'})
        if not self.ticket and self.ticket_price:
            raise ValidationError({'ticket_price': 'Price should not be set when no ticket is required.'})
    
    def save(self, *args, **kwargs):
        # Clear ticket_price if ticket is False
        if not self.ticket:
